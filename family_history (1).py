...
def main():
    ...
    # call the count marraiges function
    count_marraiges(marriages_dict, people_dict)
...
def print_death_age(people_dict):
    ...
    for item in people_dict.items():
        ...
        print(f"{name} {death_year - birth_year}")
...
def print_marriages(marriages_dict, people_dict):
    ...
    for i in marriages_dict:
        ...
        couple = marriages_dict[i]

        husband_index = couple[HUSBAND_KEY_INDEX]

        wife_index = couple[WIFE_KEY_INDEX]

        wedding_year = couple[WEDDING_YEAR_INDEX]
        ...
        h_birth = husband[BIRTH_YEAR_INDEX]

        h_age = wedding_year - h_birth
        ...
        w_birth = wife[BIRTH_YEAR_INDEX]

        w_age = wedding_year - w_birth
        print(f"{h_name}  {h_age} > {wedding_year} < {w_name} {w_age}")

    pass

def count_marraiges(marraiges_dict, people_dict):
    print("Number Of Marraiges")

    NUM_MARRAIGES_INDEX = 4
    for person_key, person_list in people_dict.items():
        person_list.append(0)
    for marraige_key, marraige_list in marraiges_dict.items():
        husband_key = marraige_list[HUSBAND_KEY_INDEX]
        wife_key = marraige_list[WIFE_KEY_INDEX]

        husband_list = people_dict[husband_key]
        husband_list[NUM_MARRAIGES_INDEX] += 1

        wife_list = people_dict[wife_key]
        wife_list[NUM_MARRAIGES_INDEX] += 1
  

    for person_key, person_list in people_dict.items():
        name = person_list[NAME_INDEX]
        num_marraiges = person_list[NUM_MARRAIGES_INDEX]
        print(f"{name} {num_marraiges}")
