...
def main():
    ...
    computed_formula = parse_formula(formula, table)

    # Call the compute_molar_mass function to compute the
    # molar mass of the molecule from the compound list.
    molar = compute_molar_mass(computed_formula, table)

    # Compute the number of moles in the sample.
    Number_of_moles = mass_in_grams / molar
    
    # Call the get_formula_name function from your main function. 
    compound_name = get_formula_name(formula, known_molecules_dict)

    # print the compound name for the user to see with the other output.
    print(f"Compound Name: {compound_name}")

    # Print the molar mass.
    print(f"{molar:.5f} grams/mole")
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    #   Split the element into symbol and quantity.
        symbol = elements[NAME_INDEX]

        quantity = elements[ATOMIC_MASS_INDEX]

    #   Get the atomic mass for the symbol.
        atom = periodic_table_dict[symbol]

        atomic_mass = atom[ATOMIC_MASS_INDEX]

    #   Multiply the atomic mass by the quantity.
        multiplication = atomic_mass * quantity

    #   Add the product into the total mass.
        total_mass = multiplication + total_mass
